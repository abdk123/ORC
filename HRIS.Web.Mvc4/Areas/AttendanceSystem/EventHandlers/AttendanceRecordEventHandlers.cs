...
namespace Project.Web.Mvc4.Areas.AttendanceSystem.EventHandlers
{
    public class AttendanceRecordEventHandlers : ViewModel
    {
            ...
            Entity parententity = null)
        {
            ...
            recordCount = typeof(AttendanceRecord).GetAll<AttendanceRecord>().Count(x => x.Id != attendanceRecord.Id && x.ToDate >= attendanceRecord.FromDate);
            if (recordCount > 0)
            {
                validationResults.Add(new ValidationResult
                {
            ...
            recordCount = typeof (AttendanceRecord).GetAll<AttendanceRecord>().Count( x => x.Id != attendanceRecord.Id && x.FromDate < attendanceRecord.FromDate && x.AttendanceMonthStatus != AttendanceMonthStatus.Locked);
                if (recordCount > 0)
                {
                    validationResults.Add(new ValidationResult
                    {
